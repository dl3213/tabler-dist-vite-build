...
function check() {
	if (getEstFileSize(false)/1024 >= 512 && getEstFileSize(false)/1024 < 513){
		shortened = true;
		document.getElementById("resolutionNotice").innerHTML = "调整到 "+(width-4)+"x"+height;
...
setInterval(function(){
	...
	if (filesize < 512)
		document.getElementById('filesizee').innerHTML = "估算的文件大小: <span style='color:green'>"+filesize+"</span> [KB]";
	else
		document.getElementById('filesizee').innerHTML = "估算的文件大小: <span style='color:red'>"+filesize+"</span> [KB]";
...
function generateCanvas(ccanvas, cwidth, cheight) {
	...
	document.getElementById('filesizee').innerHTML = "估算的文件大小: "+(getEstFileSize()/1024);
}
...
// 保持向后兼容：createVTF 调用 createVTFData 并下载
function createVTF() {
	var file = createVTFData();
	download(file, "vtf");
}
...
function getEstFileSize(cmipmaps) {
	...
	if (cmipmaps) {
		var m = document.getElementById("mipmapsCheck") || document.getElementById("vtf-mipmaps");
		if (m && m.checked) {
			mult *= 1.33203125;
		}
	}
